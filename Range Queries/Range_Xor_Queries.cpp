...
public:
    ...
    void build(v<ll> &arr, ll s, ll e, ll i = 0)
    {
        if (s == e)
        {
            tree[i] = arr[s];
    ...
    ll getXor(ll s, ll e, ll l, ll r, ll i = 0)
    {
        if (l <= s && r >= e)
        {
            return tree[i];
        ...
        if (e < l || s > r)
            return 0;
        ll mid = (s + e) / 2;
        return getXor(s, mid, l, r, 2 * i + 1) ^ getXor(mid + 1, e, l, r, 2 * i + 2);
    }
};
void solve()
{
    ...
    {
        cin >> l >> r;
        cout << tree.getXor(0, n - 1, l - 1, r - 1) << endl;
    }
}
